model{
  ...
  delta <- equals(dprobs, 1) * 0 + equals(dprobs, 2) * 1 + equals(dprobs, 3) * 2 + equals(dprobs, 4) * 3
  ...
  for(k in 1:nsite){
    ...
    for(t in 2:nyear){
      logit(psi[k,t]) <- (z[k,t-1] * (p0 + py[t-1])) +
        ((1-z[k,t-1]) * (g0 + gy[t-1]))+
        (1 - z[k,t-1] * (theta/(1*pi))*sin((pi*1)/4)*cos((pi*1 *((t-1)-delta))/2))+
        (1 - z[k,t-1] * (theta/(2*pi))*sin((pi*2)/4)*cos((pi*2 *((t-1)-delta))/2))+
        (1 - z[k,t-1] * (theta/(3*pi))*sin((pi*3)/4)*cos((pi*3 *((t-1)-delta))/2))
      z[k,t] ~ dbern(psi[k,t])
    }
  }
